...
#define MSG_NOT_YOUR_TURN (char) 0 // 服务器通知玩家在非他回合发出了出牌/质疑/UNO请求, 无 content
...
#define MSG_SYNC_GAME_INFO (char) 7 // 查询游戏信息, 无 content
#define MSG_SYNC_GAME_INFO_RESULT (char) 8 // 游戏信息同步消息回复, content 为 发送玩家数(len:1), 查询者自身的玩家序号(len:1), 各个玩家的手牌数(len:玩家数*3)，当前回合玩家序号(len:1)，弃牌堆顶层的牌号(len:3)
...
#define MSG_PLUS2 (char) 12 // PLUS2 牌被打出, content 为被起作用的玩家序号(len:1), 两个被抽到的牌号(len:2*3) 非该玩家的客户端应忽略该牌号
#define MSG_PLUS4 (char) 13 // PLUS4 牌被打出, content 为被加四张牌的玩家序号(len:1), 被加上的牌的牌号(len4*3) 非该玩家的客户端应忽略牌号
#define MSG_CHANGE_COLOR (char) 14 // CHANGE_COLOR 牌被打出, content 为新的可用颜色(len:1)
#define MSG_PASS (char) 15 // 玩家选择不出牌, 过牌, 无 content
#define MSG_PLAYER_PASSED (char) 16 // 服务器广播玩家过牌, content 为过牌玩家序号(len:1)
#define MSG_GAME_OVER (char) 17 // 游戏结束, content 为每位玩家的得分(len:5)连接，按玩家序号排序
...
#define MSG_UNO_FAILED (char) 22 // 服务器通知尝试UNO的玩家UNO失败
